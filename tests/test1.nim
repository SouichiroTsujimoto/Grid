...
suite "plus":
  test "plus(1, 3)":
    ...
    check(makeCppCode(program[0], 0, true).findStr("_k_add ( 1 ) ( 3 )"))
  
suite "minu":
  test "minu(4, plus(3, 2))":
    initTables()
    let program = "minu(4, plus(3, 2))".makeAST().astShaping()
    check(makeCppCode(program[0], 0, true).findStr("_k_sub ( 4 ) ( _k_add ( 3 ) ( 2 ) )"))

suite "mult":
  test "mult(minu(100, plus(10, 10)), 2)":
    initTables()
    let program = "mult(minu(100, plus(10, 10)), 2)".makeAST().astShaping()
    check(makeCppCode(program[0], 0, true).findStr("_k_mul ( _k_sub ( 100 ) ( _k_add ( 10 ) ( 10 ) ) ) ( 2 )"))

suite "divi":
  test "divi(365, mult(minu(100, plus(10, 10)), 2))":
    initTables()
    let program = "divi(365, mult(minu(100, plus(10, 10)), 2))".makeAST().astShaping()
    check(makeCppCode(program[0], 0, true).findStr("_k_div ( 365 ) ( _k_mul ( _k_sub ( 100 ) ( _k_add ( 10 ) ( 10 ) ) ) ( 2 ) ) ;"))

suite "|>":
  test "\"Hello\" |> puts()":
    initTables()
    let program = "\"Hello\" |> puts()".makeAST().astShaping()
    var res = ""
    for tree in program:
  ...
  test "(3 |> plus(10)) + (1 |> plus(1)) |> puts()":
    initTables()
    let program = "(3 |> plus(10)) + (1 |> plus(1)) |> puts()".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
...
suite "if":
  test "if 1 + 1 <= 3 do puts(\"OK\") end":
    initTables()
    let program = "if 1 + 1 <= 3 do puts(\"OK\") end".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
  ...
  test "if 5 + 5 == 10 do puts(\"5 + 5 = 10\") else puts(\"?\")":
    initTables()
    let program = "if 5 + 5 == 10 do puts(\"5 + 5 = 10\") else puts(\"?\")".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
  ...
  test "if 1 == 3 do puts(\"ok\") elif 4 != 5 do puts(True) elif False do puts(\"違う\") else puts(\"else\") end":
    initTables()
    let program = "if 1 == 3 do puts(\"ok\") elif 4 != 5 do puts(True) elif False do puts(\"違う\") else puts(\"else\") end".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
...
suite "puts":
  test "puts(\"Hello\")":
    initTables()
    let program = "puts(\"Hello\")".makeAST().astShaping()
    let res = makeCppCode(program[0], 0, true)
    check(res.findStr("_k_puts ( \"Hello\" ) ;"))
  test "puts(2005)":
    initTables()
    let program = "puts(2005)".makeAST().astShaping()
    let res = makeCppCode(program[0], 0, true)
    check(res.findStr("_k_puts ( 2005 ) ;"))
  test "puts(True)":
    initTables()
    let program = "puts(True)".makeAST().astShaping()
    let res = makeCppCode(program[0], 0, true)
    check(res.findStr("_k_puts ( true ) ;"))
  test "puts(\'Q\')":
    initTables()
    let program = "puts(\'Q\')".makeAST().astShaping()
    let res = makeCppCode(program[0], 0, true)
    check(res.findStr("_k_puts ( \'Q\' ) ;"))
  test "let #char ch = \'Q\' puts(ch)":
    initTables()
    let program = "let #char ch = \'Q\' puts(ch)".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
...
suite "[]":
  test "let #array #int a = {1, 2} puts(a!!1)":
    initTables()
    let program = "let #array #int a = {1, 2} puts(a!!1)".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
  ...
  test "let #array #array #int a = {{1, 2}, {3, 4}} puts(a!!1!!0)":
    initTables()
    let program = "let #array #array #int a = {{1, 2}, {3, 4}} puts(a!!1!!0)".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
...
suite "len":
  test "mut #array #int a = {1, 2} puts(len(a))":
    initTables()
    let program = "mut #array #int a = {1, 2} puts(len(a))".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
    check(res.findStr("std::vector<int> a = { 1 , 2 } ;"))
  ...
  test "mut #array #int a = {1, 2} a |> len() |> puts()":
    initTables()
    let program = "mut #array #int a = {1, 2} a |> len() |> puts()".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
    check(res.findStr("std::vector<int> a = { 1 , 2 } ;"))
...
suite "for":
  test "for #string a <- {\"a\", \"b\", \"c\"} do puts(a) end":
    initTables()
    let program = "for #string a <- {\"a\", \"b\", \"c\"} do puts(a) end".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
  ...
  test "for #string a <- {\"a\", \"b\", \"c\"} do for #string b <- {\"a\", \"b\", \"c\"} do for #string c <- {\"a\", \"b\", \"c\"} do puts(c) end end end":
    initTables()
    let program = "for #string a <- {\"a\", \"b\", \"c\"} do for #string b <- {\"a\", \"b\", \"c\"} do for #string c <- {\"a\", \"b\", \"c\"} do puts(c) end end end".makeAST().astShaping()
    var res = ""
    for tree in program:
      res.add(makeCppCode(tree, 0, true))
    check(res.findStr("for ( std::string a : { \"a\" , \"b\" , \"c\" } ) {"))
